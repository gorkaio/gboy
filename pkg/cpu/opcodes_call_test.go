...
func TestCallZJumpsAndSetsSPWhenZeroFlagSet(t *testing.T) {
	tests := []testDescription{
		{
			description:      "'CALL Z, a16' jumps to a16 address with PC in SP if Zero Flag set",
			opcode:           opcode{0xCC, 0x34, 0x12},
			regsGiven:        regMap{"PC": 0x100, "SP": 0x402, "F": FlagZ},
			regsExpected:     regMap{"PC": 0x1234, "SP": 0x400},
			memReadExpected:  memMap{},
			memWriteExpected: memMap{0x401: 0x01, 0x400: 0x00},
			cycles:           24,
		...
		{
			description:      "'CALL Z, a16' does not jump to a16 if Zero Flag clear",
			opcode:           opcode{0xCC, 0x34, 0x12},
...
func TestCallCJumpsAndSetsSPWhenCarryFlagSet(t *testing.T) {
	tests := []testDescription{
		{
			description:      "'CALL C, a16' jumps to a16 address with PC in SP if Carry Flag set",
			opcode:           opcode{0xDC, 0x34, 0x12},
			regsGiven:        regMap{"PC": 0x100, "SP": 0x402, "F": FlagC},
			regsExpected:     regMap{"PC": 0x1234, "SP": 0x400},
			memReadExpected:  memMap{},
			memWriteExpected: memMap{0x401: 0x01, 0x400: 0x00},
			cycles:           24,
		...
		{
			description:      "'CALL C, a16' does not jump to a16 if Carry Flag clear",
			opcode:           opcode{0xDC, 0x34, 0x12},
			regsGiven:        regMap{"PC": 0x216},
			regsExpected:     regMap{"PC": 0x219},
			memReadExpected:  memMap{},
