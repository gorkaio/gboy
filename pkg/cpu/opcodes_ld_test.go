...
func TestLoadImmediateFor8bitRegisters(t *testing.T) {
	registerOpcodes := map[string]opcode{
		"A": {0x3E, 0x12},
		"B": {0x06, 0x12}, "C": {0x0E, 0x12},
		"D": {0x16, 0x12}, "E": {0x1E, 0x12},
		"H": {0x26, 0x12}, "L": {0x2E, 0x12},
	}
...
func TestLoadImmediateFor16bitRegisters(t *testing.T) {
	registerOpcodes := map[string]opcode{
		"BC": {0x01, 0x34, 0x12}, "DE": {0x11, 0x34, 0x12},
		"HL": {0x21, 0x34, 0x12}, "SP": {0x31, 0x34, 0x12},
	}
...
func TestLoadIndirectOther(t *testing.T) {
	testDescriptions := []testDescription{
		...
		{
			"'LD (a16), A' loads A into memory address a16",
			opcode{0xEA, 0x34, 0x12},
			regMap{"A": 0x56},
			regMap{},
			memMap{},
			memMap{0x1234: 0x56},
		...
		{
			"'LDH (a8), A' loads A into memory address (a8 + 0xFF00)",
			opcode{0xE0, 0x12},
			regMap{"A": 0x56},
			regMap{},
			memMap{},
		...
		{
			"'LDH A, (a8)' loads memory address content (a8 + 0xFF00) into A",
			opcode{0xF0, 0x12},
			regMap{},
			regMap{"A": 0x56},
			memMap{0xFF12: 0x56},
		...
		{
			"'LDHL SP, r8' loads memory address content (SP + r8) into HL. Clears Z and N flags.",
			opcode{0xF8, 0x12},
			regMap{"SP": 0x1234, "F": FlagZ | FlagN},
			regMap{"HL": 0x5678, "F": 0},
			memMap{0x1246: 0x78, 0x1247: 0x56},
			memMap{},
			12,
		},
		{
			"'LD SP, HL' loads HL into SP",
			opcode{0xF9},
			regMap{"HL": 0x1234},
			regMap{"SP": 0x1234},
			memMap{},
		...
		{
			"'LD A, (a16)' loads content from memory address a16 into A",
			opcode{0xFA, 0x34, 0x12},
			regMap{},
			regMap{"A": 0x56},
			memMap{0x1234: 0x56},
			memMap{},
